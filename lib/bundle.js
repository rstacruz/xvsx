...
function inlineMarked (src) {
  return marked(src).replace(/<p>|<\/p>\n/g,'')
}
function highlight (lang, text) {
  ...
  html = html
    ...
    .replace(
      /<span class="hljs-comment">[\/\#\*]+! (.*?)(?: \*\/)?<\/span>/g,
      (_, comment) => `<span class="hljs-comment -mini">${inlineMarked(comment)}</span>`)
    .replace(
      /<span class="hljs-comment">[\/\#\*]+=&gt; (.*?)<\/span>/g,
    ...
    .replace(
      /\n+\s*<span class="hljs-comment">[\/\#]+ doc: (.*?)<\/span>/g,
      (_, href) => `<a class="hljs-doclink" href="${href}"></a>`)
  return html
}
